...
/**
 * What to call a voice. Note keys are always a single character; the drone's
 * voices are the only other thing that reaches the audio layer, and they arrive
 * as ids rather than keys.
 */
const labelForVoice = ({ key, sustained }) => {
  const name = key?.length === 1 ? key : 'drone';
 ...
 */
const pairsFrom = (voices) => {
  const sorted = [...voices].sort((first, second) => first.frequency - second.frequency);

  // Two voices at the same pitch do not beat, they sum. A row reading
  // "1/1, just, 0 Hz" is not information.
...
const rowsFor = (pairs, shape) => pairs.flatMap(([lower, upper]) => {
  const interval = describeInterval(lower.frequency, upper.frequency, { waveShape: shape });

  if (!interval) return [];

  return [{
    lowerLabel: labelForVoice(lower),
    upperLabel: labelForVoice(upper),
    interval,
  }];
});
...
const draw = () => {
  ...
  const rows = rowsFor(pairsFrom(voices), shape);
